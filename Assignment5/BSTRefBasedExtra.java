 ...
 * This is the extra reference-based implementation of the ADT binary search tree.
 * The extra part is to calculate the height of the BST
 */

import java.util.Iterator;

public class BSTRefBasedExtra extends AbstractBinaryTree implements Iterable<WordRefs> {
    ...
    /**
     * Constructor for BSTRefBasedExtra
     * @param item
     * @param left
     * @param right
     */
    public BSTRefBasedExtra(WordRefs item, AbstractBinaryTree left, AbstractBinaryTree right) {
    ...
    public AbstractBinaryTree detachLeftSubtree() throws TreeException {
        ...
        BSTRefBasedExtra result = new BSTRefBasedExtra();
    ...
    /**
    * Extra part for calculate the height of the BST
    * if the node is null object, returns -1.
    * @param node
    */
    public int calcHeightExtra(TreeNode node) {

        if(node == null) {
            return 0;
        }
        if(calcHeightExtra(node.left) > calcHeightExtra(node.right)) {
            return calcHeightExtra(node.left) + 1;
        } else {
            return calcHeightExtra(node.right) + 1;
        }
    }
    ...
    public Iterator<WordRefs> iterator() {
        return new BSTIteratorExtra(this);
    }
    ...
    public static void main(String args[]) {
       ...
       BSTRefBasedExtra t1 = new BSTRefBasedExtra();
       if(t1.isEmpty()) {
       ...
       BSTRefBasedExtra t2 = new BSTRefBasedExtra();
       t2.insert("blue");
       t2.insert("green");
       t2.insert("azure");
       t2.insert("cornsilk");  
       ...
       BSTRefBasedExtra t3 = new BSTRefBasedExtra();
       t3.insert("blue");
       t3.insert("green");
       t3.insert("azure");
       t3.insert("cornsilk"); 
       ...
       if(retrieveNode3.item.getWord().equals("azure")) {
    	   System.out.println("\nTest 6 of 'findLeftMost' passed.");
       } else {
    	   System.out.println("\nTest 6 of 'findLeftMost' FAILED.");
       }

       BSTRefBasedExtra t4 = new BSTRefBasedExtra();
       t4.insert("blue");
       t4.insert("green");
       t4.insert("azure");
       t4.insert("cornsilk"); 
       t4.insert("black");
       t4.insert("aquamarine");
       t4.insert("turquoise");
       t4.insert("antiquewhite");
       if(t4.calcHeightExtra(t4.root) == 4) {
            System.out.println("\nTest 7 of 'calcHeightExtra' passed.");
       } else {
            System.out.println("\nTest 7 of 'calcHeightExtra' FAILED.");
       }
    }
} 
